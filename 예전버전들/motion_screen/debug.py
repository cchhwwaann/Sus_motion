...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        try:
            ...
            # 모델이 (1, 213, 63) 형태의 입력을 기대하므로, 더미 시퀀스 생성
            landmarks_array = np.array([[lm.x, lm.y, lm.z] for lm in landmarks]).flatten()  # (63,)
            input_data = np.tile(landmarks_array, (213, 1))  # (213, 63)
            input_data = input_data.reshape(1, 213, 63)  # (1, 213, 63)
            prediction = model.predict(input_data)
            gesture = int(np.argmax(prediction))
            # gesture가 0이면 커서 이동만, 1이면 클릭(한 번만) 실행
            ...
            # 제스처에 따른 동작: 
            # gesture가 1이면(클릭 제스처) 한 번만 클릭하도록 처리
            if gesture == 1:
